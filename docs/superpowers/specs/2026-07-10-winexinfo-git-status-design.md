...
- 검토 상태: 대화에서 승인된 설계를 문서화했으며 형님의 문서 검토를 기다린다.
...
| `explorer.exe` | `10.0.26100.8457` |
| `ExplorerFrame.dll` | `10.0.26100.8457` |
| `shell32.dll` | `10.0.26100.8521` |
...
4. `IShellBrowser::QueryActiveShellView`와 `IShellView::GetWindow`로 view HWND를 얻는다.
5. view HWND가 현재 활성 `ShellTabWindowClass`의 descendant이며 실제로 보이는 entry를 고른다.
6. 일치하는 entry가 정확히 하나일 때만 그 view를 활성 view로 확정한다.
7. `IFolderView::GetFolder`로 현재 폴더의 `IShellFolder`를 얻는다.
8. `SHGetIDListFromObject`로 absolute PIDL을 얻는다.
9. `SHCreateItemFromIDList`로 `IShellItem`을 만든다.
10. `SIGDN_FILESYSPATH` 파일 시스템 경로를 요청한다.
...
탭 selection event에서는 기존 표시를 즉시 숨기고 active view를 다시 판정한다. view가 아직 정확히 하나가 아니면 주기적으로 재시도하지 않고 다음 NavigateComplete2 또는 structure-changed event까지 숨김을 유지한다. event 뒤에도 1~10단계가 0개 또는 여러 view를 반환하면 `ACTIVE_VIEW_CONTRACT_MISMATCH`를 기록한다.

read-only `--probe`는 다중 탭 창에서 위 각 event가 active view와 정확히 연결되고 1~10단계가 정확히 하나의 view를 반환함을 입증해야 한다. 이 매핑이 성립하지 않으면 주소창, 탭 제목, UIA Name을 대체 key로 사용하지 않고 설계를 다시 결정한다.
...
- `ShellTabWindowClass`, `DUIViewWndClassName`, `DirectUIHWND` parent chain
...
Gate A와 B가 통과한 뒤 형님의 별도 승인을 받아 현재 Explorer 창 하나에만 실행한다. Git, watcher, 자동 실행, 설치는 포함하지 않고 고정된 `WinExInfo POC` 문자열만 표시한다.
...
3. 형님의 별도 승인 후 실제 Explorer E2E가 모두 통과한다.
