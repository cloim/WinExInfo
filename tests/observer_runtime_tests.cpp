...
WXI_TEST(
    observer_runtime_findwindow_uses_exact_cookie_contract,
    "observer_runtime.findwindow_cookie_contract") {
    FakeDispatch dispatch;
    bool called = false;
    const winexinfo::ShellWindowResolverOperations operations{
        [&](VARIANT* const location,
            VARIANT* const root,
            const int shellClass,
            long* const legacyHwnd,
            const int flags,
            IDispatch** const output) {
            called = true;
    ...
    Microsoft::WRL::ComPtr<IDispatch> resolved;
    const winexinfo::ObserverOperationResult result =
        winexinfo::FindRegisteredShellDispatch(73, operations, resolved);
    WXI_REQUIRE(result.ok());
    WXI_REQUIRE(!result.failure_origin.has_value());
    WXI_REQUIRE(called);
    WXI_REQUIRE_EQ(resolved.Get(), static_cast<IDispatch*>(&dispatch));
}

WXI_TEST(
    observer_runtime_findwindow_does_not_guess_deferred_cookie,
    "observer_runtime.findwindow_cookie_deferred_failure") {
    for (const HRESULT deferred : {S_FALSE, E_PENDING}) {
        std::size_t calls = 0;
        const winexinfo::ShellWindowResolverOperations operations{
            [&](VARIANT* const location,
                VARIANT* const root,
                ...
                WXI_REQUIRE(location != nullptr);
                WXI_REQUIRE_EQ(location->vt, VT_I4);
                WXI_REQUIRE_EQ(location->lVal, LONG{91});
                WXI_REQUIRE(root != nullptr);
                WXI_REQUIRE_EQ(root->vt, VT_EMPTY);
                WXI_REQUIRE_EQ(shellClass, int{SWC_EXPLORER});
                WXI_REQUIRE(legacyHwnd != nullptr);
                WXI_REQUIRE_EQ(
                    ...
                    int{SWFO_COOKIEPASSED | SWFO_NEEDDISPATCH |
                        SWFO_INCLUDEPENDING});
                WXI_REQUIRE(output != nullptr);
                return deferred;
            },
        };
        Microsoft::WRL::ComPtr<IDispatch> resolved;
        const winexinfo::ObserverOperationResult result =
            winexinfo::FindRegisteredShellDispatch(91, operations, resolved);
        WXI_REQUIRE(!result.ok());
        WXI_REQUIRE_EQ(
            result.failure_origin,
            std::optional{winexinfo::ObserverFailureOrigin::Contract});
        WXI_REQUIRE_EQ(result.status.hresult, deferred);
        WXI_REQUIRE_EQ(calls, std::size_t{1});
        WXI_REQUIRE(resolved == nullptr);
    }
}
...
WXI_TEST(
    observer_runtime_findwindow_preserves_typed_failure_and_output,
    "observer_runtime.findwindow_typed_failure") {
    ...
    const HRESULT structuralResults[] = {S_FALSE, E_PENDING, E_NOINTERFACE};
    for (const HRESULT hresult : structuralResults) {
        const winexinfo::ShellWindowResolverOperations operations{
            [hresult](VARIANT*, VARIANT*, int, long*, int, IDispatch**) {
                return hresult;
            },
        };
        const auto result =
            winexinfo::FindRegisteredShellDispatch(73, operations, output);
        WXI_REQUIRE(!result.ok());
        WXI_REQUIRE_EQ(result.status.hresult, hresult);
        WXI_REQUIRE_EQ(
            result.failure_origin,
            std::optional{winexinfo::ObserverFailureOrigin::Contract});
        WXI_REQUIRE(output.Get() != nullptr);
        WXI_REQUIRE_EQ(output.Get(), static_cast<IDispatch*>(&sentinel));
    }
    const winexinfo::ShellWindowResolverOperations transportOperations{
        [](VARIANT*, VARIANT*, int, long*, int, IDispatch**) {
            return RPC_E_DISCONNECTED;
        },
    };
    const auto transport = winexinfo::FindRegisteredShellDispatch(
        73, transportOperations, output);
    WXI_REQUIRE_EQ(transport.status.hresult, RPC_E_DISCONNECTED);
    WXI_REQUIRE_EQ(
        transport.failure_origin,
        std::optional{winexinfo::ObserverFailureOrigin::Transport});
    WXI_REQUIRE_EQ(output.Get(), static_cast<IDispatch*>(&sentinel));
}

WXI_TEST(
    observer_runtime_findwindow_failed_hresult_discards_illegal_dispatch,
    "observer_runtime.findwindow_mixed_failure_output") {
    FakeDispatch sentinel;
    FakeDispatch illegalDispatch;
    Microsoft::WRL::ComPtr<IDispatch> output = &sentinel;
    WXI_REQUIRE_EQ(output.Get(), static_cast<IDispatch*>(&sentinel));
    const ULONG baselineReferences = illegalDispatch.references();
    const winexinfo::ShellWindowResolverOperations operations{
        [&illegalDispatch](
            VARIANT*,
            VARIANT*,
            ...
            long*,
            int,
            IDispatch** const dispatchOutput) {
            *dispatchOutput = &illegalDispatch;
            illegalDispatch.AddRef();
            return RPC_E_DISCONNECTED;
        },
    };
    const auto result =
    ...
    WXI_REQUIRE(output.Get() != nullptr);
    WXI_REQUIRE_EQ(output.Get(), static_cast<IDispatch*>(&sentinel));
    WXI_REQUIRE_EQ(illegalDispatch.references(), baselineReferences);
}
...
WXI_TEST(
    observer_runtime_shell_set_transition_is_order_independent_and_exact,
    "observer_runtime.shell_set_transition") {
    ...
    const std::array<winexinfo::ObserverShellEntryMetadata, 2> reordered{
        previous[1], previous[0]};
    winexinfo::ObserverShellSetTransition transition{
        winexinfo::ObserverShellEntryMetadata{
            99, false, Handle(0x900), nullptr},
        std::nullopt,
    };
    WXI_REQUIRE(winexinfo::ClassifyObserverShellSetTransition(
                    winexinfo::ObserverShellTransitionKind::Stable,
                    previous,
                    reordered,
                    0,
                    &transition)
                    .ok());
    WXI_REQUIRE(!transition.added.has_value());
    WXI_REQUIRE(!transition.removed.has_value());

    const std::array<winexinfo::ObserverShellEntryMetadata, 3> added{
        previous[1],
        winexinfo::ObserverShellEntryMetadata{
            33, true, Handle(0x300), Handle(0x301)},
        previous[0],
    };
    WXI_REQUIRE(winexinfo::ClassifyObserverShellSetTransition(
                    winexinfo::ObserverShellTransitionKind::Registered,
                    previous,
                    added,
                    33,
                    &transition)
                    .ok());
    WXI_REQUIRE_EQ(transition.added->canonical_identity, std::uintptr_t{33});
    WXI_REQUIRE(!transition.removed.has_value());
    WXI_REQUIRE(winexinfo::ClassifyObserverShellSetTransition(
                    winexinfo::ObserverShellTransitionKind::Revoked,
                    added,
                    previous,
                    0,
                    &transition)
                    .ok());
    WXI_REQUIRE_EQ(transition.removed->canonical_identity, std::uintptr_t{33});
}

WXI_TEST(
    observer_runtime_shell_set_transition_rejects_metadata_and_simultaneous_delta,
    "observer_runtime.shell_set_transition_failures") {
    const std::array<winexinfo::ObserverShellEntryMetadata, 2> previous{
        winexinfo::ObserverShellEntryMetadata{
            11, true, Handle(0x100), Handle(0x101)},
        winexinfo::ObserverShellEntryMetadata{
    ...
    auto mutated = previous;
    mutated[0].shell_tab = Handle(0x102);
    winexinfo::ObserverShellSetTransition unchanged{
        winexinfo::ObserverShellEntryMetadata{
            99, false, Handle(0x900), nullptr},
        std::nullopt,
    };
    RequireActiveFailure(
        winexinfo::ClassifyObserverShellSetTransition(
            winexinfo::ObserverShellTransitionKind::Stable,
            previous,
            mutated,
            0,
            &unchanged),
        S_FALSE);
    WXI_REQUIRE_EQ(unchanged.added->canonical_identity, std::uintptr_t{99});

    const std::array<winexinfo::ObserverShellEntryMetadata, 2> simultaneous{
        previous[0],
        winexinfo::ObserverShellEntryMetadata{
            33, false, Handle(0x300), nullptr},
    };
    RequireActiveFailure(
        winexinfo::ClassifyObserverShellSetTransition(
            winexinfo::ObserverShellTransitionKind::Registered,
...
WXI_TEST(
    observer_runtime_shell_set_transition_handles_nontarget_and_rejects_bad_shape,
    "observer_runtime.shell_set_transition_shape") {
    const std::array<winexinfo::ObserverShellEntryMetadata, 1> previous{
        winexinfo::ObserverShellEntryMetadata{
            11, true, Handle(0x100), Handle(0x101)},
    };
    const std::array<winexinfo::ObserverShellEntryMetadata, 2> withNonTarget{
        previous[0],
        winexinfo::ObserverShellEntryMetadata{
            22, false, Handle(0x200), nullptr},
    };
    winexinfo::ObserverShellSetTransition output{};
    WXI_REQUIRE(winexinfo::ClassifyObserverShellSetTransition(
                    winexinfo::ObserverShellTransitionKind::Registered,
                    previous,
                    withNonTarget,
                    ...
                    &output)
                    .ok());
    WXI_REQUIRE(!output.added->target_matched);
    WXI_REQUIRE(winexinfo::ClassifyObserverShellSetTransition(
                    winexinfo::ObserverShellTransitionKind::Revoked,
                    withNonTarget,
                    previous,
                    ...
                    &output)
                    .ok());
    WXI_REQUIRE(!output.removed->target_matched);

    const winexinfo::ObserverShellSetTransition sentinel{
        winexinfo::ObserverShellEntryMetadata{
            99, false, Handle(0x900), nullptr},
        std::nullopt,
    };
    ...
    RequireActiveFailure(
        winexinfo::ClassifyObserverShellSetTransition(
            static_cast<winexinfo::ObserverShellTransitionKind>(99),
            previous,
            previous,
            0,
            &output),
    ...
    const std::array<winexinfo::ObserverShellEntryMetadata, 1> zeroIdentity{
        winexinfo::ObserverShellEntryMetadata{
            0, true, Handle(0x100), Handle(0x101)},
    };
    RequireActiveFailure(
        winexinfo::ClassifyObserverShellSetTransition(
            winexinfo::ObserverShellTransitionKind::Stable,
            ...
            &output),
        S_FALSE);
    const std::array<winexinfo::ObserverShellEntryMetadata, 2> duplicate{
    ...
    RequireActiveFailure(
        winexinfo::ClassifyObserverShellSetTransition(
            ...
            &output),
        S_FALSE);
    const std::array<winexinfo::ObserverShellEntryMetadata, 1> badTargetShape{
        winexinfo::ObserverShellEntryMetadata{
            11, true, Handle(0x100), nullptr},
    };
    RequireActiveFailure(
        winexinfo::ClassifyObserverShellSetTransition(
            winexinfo::ObserverShellTransitionKind::Stable,
...
WXI_TEST(
    observer_runtime_shell_lifecycle_uses_exact_registration_identity,
    ...
    WXI_REQUIRE(correlation.target_matched);
    WXI_REQUIRE(correlation.new_top_level);
    WXI_REQUIRE_EQ(correlation.entry.canonical_identity, std::uintptr_t{33});
    WXI_REQUIRE_EQ(correlation.entry.top_level, exactNewTop);
    WXI_REQUIRE_EQ(correlation.generation, std::uint64_t{5});
    WXI_REQUIRE_EQ(correlation.top_level_entry_count, std::size_t{1});
...
WXI_TEST(
    observer_runtime_shell_lifecycle_rejects_registration_without_exact_identity,
    "observer_runtime.shell_lifecycle_correlation.unresolved_registration") {
    const HWND existingTop = Handle(0x100);
    const HWND addedTop = Handle(0x200);
    const std::array<winexinfo::ObserverShellEntryMetadata, 1> previous{
        winexinfo::ObserverShellEntryMetadata{
            11, true, existingTop, Handle(0x101)},
    };
